...
from giga_funcs import each_post_summary
import asyncio
import nest_asyncio
import pandas as pd
...
modal_search = Modal("Исторический поиск", key="history_search")
modal_wait = Modal("Идет суммаризация", key="wait")
...
def collect_news(csv_file, keyword_h, start_date, end_date, document_path, threshold=76):
    # Загрузка CSV файла
    #df = pd.read_excel(csv_file, sep=',', usecols=['date', 'text'])
    df = pd.read_excel(csv_file, usecols=['date', 'text'])
    ...
    def is_keyword_in_news(news_text, keyword_h, threshold):
        ...
        keyword_h_lower = keyword_h.lower()
        news_text_lower = news_text.lower()
    
    # Проверяем на точное вхождение или порог схожести
        return keyword_h_lower in news_text_lower or fuzz.partial_ratio(keyword_h_lower, news_text_lower) >= threshold
...
def update_progress(total_posts, processed_posts):
    progress = max((processed_posts / total_posts)-0.1, 0.05)
    my_bar=progress_placeholder.progress(0)
    my_bar.progress(progress)

if modal_search.is_open():
    with modal_search.container():
        if st.session_state['search_started']== True:
            st.write("Поиск начат. Пожалуйста, подождите...")
            progress_placeholder=st.empty()
            # Код ниже нужен для очистки кеша модального окна (стереть старые поля)
            st.write("")
            st.write("")
            st.write("")
            st.write("")
            st.session_state['search_started']= False
            if st.session_state['keyword_h'] == 'all_posts':
                #sum_summary, total_posts, processed_posts=asyncio.run(posts_summary(document_path, prompt_type='prompt_all_posts', keyword=None, update_progress_callback=update_progress))
                sum_summary=asyncio.run(posts_summary(document_path, prompt_type='prompt_all_posts', keyword=None, update_progress_callback=update_progress))
            else:
                #sum_summary=asyncio.run(posts_summary(document_path, prompt_type='prompt_key_word', keyword=st.session_state['keyword_h'], update_progress_callback=update_progress))
                sum_summary=asyncio.run(each_post_summary(document_path, prompt_type='prompt_key_word', keyword=st.session_state['keyword_h'], update_progress_callback=update_progress))
...
if prompt := st.chat_input():
    ...
    if result.additional_kwargs and result.additional_kwargs.get('function_call', {}).get('name') == 'news':
